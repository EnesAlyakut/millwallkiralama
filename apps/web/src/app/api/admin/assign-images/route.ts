...
export async function GET(req: Request) {
  try {
    ...
    for (const item of mapping) {
      ...
      const sourceExt = path.join(ARTIFACTS_DIR, item.ext);
      if (fs.existsSync(sourceExt)) {
        fs.copyFileSync(sourceExt, path.join(UPLOADS_DIR, extDest));
      }

      let intDest = null;
      if (item.int) {
        intDest = item.slug + '-int.jpg';
        const sourceInt = path.join(ARTIFACTS_DIR, item.int);
        if (fs.existsSync(sourceInt)) {
          fs.copyFileSync(sourceInt, path.join(UPLOADS_DIR, intDest));
        }
      }
